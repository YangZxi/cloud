  ...
  <div id='user-login'>
    ...
    <div id="login-card">
      <div style="min-width: 300px; padding: 20px 15px 0px 35px;position: relative;">
        ...
        <n-form :model="user" ref="loginForm" :show-label="false" style="position: relative;">
          <Transition name="slide" class="input-box">
            <div v-if="!guestLogin">
              <n-form-item path="username">
                <n-input v-model:value="user.username" :autofocus="true" ref="input">
                  <template #prefix>账号：</template>
                </n-input>
              </n-form-item>
              <n-form-item path="password">
                <n-input type="password" v-model:value="user.password" @keyup.enter.native="login">
                  <template #prefix>密码：</template>
                </n-input>
              </n-form-item>
            </div>
            <div v-else="guestLogin">
              <!-- <span style="font-size: 12px;margin: 20px 0;display: block;" @click="guestLogin = false">&lt; 用户登录</span> -->
              <n-form-item path="password">
                <n-input type="password" v-model:value="user.password" placeholder="请输入访问密码" ref="input"
          ...
          <n-form-item>
            <n-button type="primary" id='loginBtn' @click="login">登录</n-button>
          </n-form-item>
          <!-- <el-form-item>
        ...
        <div class="btn-group">
          <n-button type="primary" size="tiny" dashed @click="$router.push('/')">
            <template #icon>
              <n-icon>
                <AngleLeft />
              </n-icon>
          ...
          <n-button type="primary" size="tiny" dashed @click="toggle">
            <template #icon>
              <n-icon>
                <Seedling />
              </n-icon>
      ...
      <div style="flex: 1;height: 100%;box-sizing: border-box;padding: 10px;">
        <img style="width: 100%; height: 100%;background-color: #EBF4FF;" :src="bg" fit="contain" />
      </div>
    </div>
    <!-- </el-card> -->
  </div>
...
import { nextTick, onMounted, reactive, ref, Transition } from 'vue'
