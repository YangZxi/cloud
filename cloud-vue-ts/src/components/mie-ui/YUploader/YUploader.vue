          ...
          >
            <template #default="detail">
              <div style="display: flex;align-items: center;height: 100%;">
                ...
                >
                  <template #icon>
                    <y-icon :name="file.paused ? 'fa-circle-play' : 'fa-circle-pause'" />
                  </template>
                </n-button>
                <n-button
                  circle
...
export default {
    ...
    const data: {
      ...
      options: {
        ...
        query: (_file, _chunk, _isTest) => {
          // console.esLog("a:", file, chunk, isTest);
          return {
            ...this.uploadData
          };
        },
  ...
  methods: {
    ...
    onFileAdded(file, event) {
      console.esLog("eeeee", event);
      // event.preventDefault();
      console.esLog("file :>> ", file);
      // 有时 fileType为空，需截取字符
      console.esLog("文件类型：" + file.fileType + "，文件大小：" + file.size + "B");
      // 1. todo 判断文件类型是否允许上传
      ...
      this.getFileMD5(file, async (md5) => {
        if (md5 !== "") {
          ...
          uploadCheck(data).then(() => {
            this.uploadFileList.push(file);
            // 恢复上传
            file.resume();
          }).catch(() => {
            console.esLog("取消文件上传");
            file.error = true;
            file.cancel();
          });
        }
    ...
    onFileSuccess(rootFile, file, response, chunk) {
      mergeChunk({
        ...
        console.esLog("上传成功", rootFile, file, response, chunk);
    ...
    onFileError(rootFile, file, response, chunk) {
      console.esLog("上传出错：", rootFile, file, response, chunk);
      const res = JSON.parse(response);
      window.$message.error(res.msg);
    },
    onFileProgress(_rootFile, file, _chunk) {
      console.esLog(`当前进度：${Math.ceil(file._prevProgress * 100)}%`);
    },
    // 计算文件的MD5值
    getFileMD5(file, callback) {
      ...
      fileReader.onload = function(e) {
        // console.esLog("currentChunk :>> ", currentChunk);
        ...
        if (currentChunk < chunks) {
          ...
          // 该文件的md5值
          let md5 = spark.end();
          console.esLog(`MD5计算完毕：${md5}，耗时：${new Date().getTime() - startTime} ms.`);
          // 回调传值md5
          callback(md5);
        }
      };
    ...
    formatFileSize(bytes, decimalPoint = 2) {
      ...
      let i = Math.floor(Math.esLog(bytes) / Math.esLog(k));
      return (
